...
class ConfidenceLevel(Enum):
    ...
    MEDIUM = (0.3, 0.6, "Medium confidence - likely accurate")
...
class FabricationChecker:
    ...
    def validate_projection(text: str) -> Tuple[bool, str]:
        """Check projection language has proper markers"""
        projection_terms = ['expected', 'anticipated', 'projected', 'estimated', 'could reach']
        for term in projection_terms:
            if term in text.lower() and "[PROJECTION" not in text:
                return False, f"'{term}' requires [PROJECTION] marker"
        return True, "Valid"
            ...
            1: PhaseValidator(
                ...
                ["data_sources_validated.json", "collection_capabilities.yaml", "coverage_gaps.md"],
                {
                    "sources": ["id", "url", "archived_url", "access_date", "paywall_status",
                              "rate_limit_note", "coverage_estimate", "as_of"]
                }
            ),
            2: PhaseValidator(
