...
**Data Reality Check (Updated from UNIFIED_DATA_INFRASTRUCTURE_INVENTORY.md):**
- **Total Available:** 447GB (corrected from 445GB)
- **OpenAlex:** 422GB at `F:/OSINT_Backups/openalex/data/` (0.5% processed, checkpoint at 1.2M records, 68 Germany-China collaborations found)
- **TED:** 25GB at `F:/TED_Data/` (0% processed - **HIGHEST PRIORITY** for government contracts)
- **CORDIS:** 1.1GB multiple locations (100% H2020 done, found 168 Italy-China projects, 0% Horizon Europe)
- **SEC EDGAR:** 127MB at `F:/OSINT_Data/SEC_EDGAR/` (0% processed)
- **EPO Patents:** 120MB at `F:/OSINT_DATA/EPO_PATENTS/` (0% processed, Leonardo only)
- **Hardware Required:** 32GB RAM minimum for OpenAlex streaming
- **Processing Priority:** 1) TED (HIGHEST), 2) OpenAlex (resume from checkpoint), 3) Horizon Europe CORDIS
...
def adjust_confidence_for_coverage(base_confidence, coverage_percent, data_type):
    ...
    # Based on actual processing status from UNIFIED_DATA_INFRASTRUCTURE_INVENTORY.md
...
### Current Data Processing Reality (from UNIFIED_DATA_INFRASTRUCTURE_INVENTORY.md):
- **OpenAlex:** 1,225,874 papers analyzed (0.5% of 250M), found 68 Germany-China collaborations
- **TED:** 0 files processed (HIGHEST PRIORITY for government contracts)
- **CORDIS:** 168 Italy-China projects found from H2020 (complete)
- **Checkpoint available:** OpenAlex at 1.2M records in `data/processed/openalex_real_data/checkpoint.json`
...
Available: SEC filings (127MB unprocessed), patents (120MB EPO unprocessed), news.
