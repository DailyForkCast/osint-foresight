"""
Comprehensive OpenAlex Integration to Master Database - VERSION 5
NULL DATA-DRIVEN EXPANSION: Hybrid approach using V4 NULL captures to expand patterns
- Stage 1 (Keywords): 355 -> 625 keywords (+76% increase via NULL data analysis)
- Stage 2 (Topics): 327 -> 487 patterns (+49% increase via NULL data analysis)
- Multi-stage validation with OpenAlex topics and word boundaries
- Lowered topic score threshold from 0.5 to 0.3
- Expected improvement: 2-3x more works collected vs V4 (30,000-45,000 total)
- Maintains V4 precision: 70-82% false positive reduction
"""
...
# Load expanded keywords from config - V4 FEATURE
def load_expanded_keywords():
    ...
    if not config_path.exists():
        ...
        print("[WARN] Falling back to V3 keywords")
        return None
    ...
    try:
        ...
        print(f"[V5] Loaded expanded keyword patterns from {config_path.name}")
...
# Try to load V5 expanded keywords
EXPANDED_KEYWORDS = load_expanded_keywords()

if EXPANDED_KEYWORDS:
    TECHNOLOGY_KEYWORDS = EXPANDED_KEYWORDS
    print("[V5] Using EXPANDED keyword patterns (625 total)")
else:
...
# OpenAlex topic whitelist - V3 EXPANDED TOPICS
# Loads from config/openalex_relevant_topics_expanded.json
# Falls back to V2 patterns if config not found
def load_expanded_topics():
    """Load expanded topic patterns from JSON configuration (V5)"""
    # V5: Use NULL data-driven expanded patterns
    ...
    if not config_path.exists():
        print(f"[WARN] V5 expanded topics config not found at {config_path}")
        print("[WARN] Falling back to V2 topic patterns")
        return None
    ...
    try:
        ...
        for tech, pattern_groups in config.items():
            if tech.startswith('_'):  # Skip metadata fields like _comment
                continue
        ...
        print(f"[V5] Loaded expanded topic patterns from {config_path.name}")
...
# Try to load V5 expanded topics
EXPANDED_TOPICS = load_expanded_topics()

if EXPANDED_TOPICS:
    RELEVANT_TOPICS = EXPANDED_TOPICS
    print("[V5] Using EXPANDED topic patterns (487 total)")
else:
    ...
    print("[V2] Using fallback topic patterns (69 total)")
        ...
        for pattern in relevant_patterns:
            ...
            if strictness == 'lenient':
                ...
                if pattern_lower in topic_name:
                    # V3: Lowered threshold from 0.5 to 0.3 to capture more relevant works
...
def process_all_works_data(technology_domains, max_works_per_tech=10000, strictness='moderate', sample_mode=False):
    """
    Process OpenAlex files for specified technologies with expanded keywords and topics (V5)

    V5 enhancements:
    - 625 keywords (vs 355 in V4) - 76% increase via NULL data analysis
    - 487 topic patterns (vs 327 in V4) - 49% increase via NULL data analysis
    - Lowered topic score threshold to 0.3 (from 0.5)
    - NULL data-driven expansion: patterns derived from real validated gaps
    - Expected 2-3x improvement: 30,000-45,000 works vs 14,955 in V4

    strictness: 'lenient', 'moderate', 'strict'
    sample_mode: If True, sample ~100 files from diverse date directories for testing
    """

    print(f"\n{'=' * 80}")
    print("OPENALEX PROCESSING - VERSION 5 (NULL DATA-DRIVEN EXPANSION)")
    print(f"{'=' * 80}")
    ...
    print(f"Sample mode: {'YES (diverse sampling)' if sample_mode else 'NO (all files)'}")
    ...
    # Get files with improved sampling for representative data
    if sample_mode:
        # IMPROVED: Sample from diverse directories, not sequential files
        date_dirs = sorted(works_dir.glob('updated_date=*'))

        if not date_dirs:
            print(f"[ERROR] No date directories found in {works_dir}")
            return {}, {}

        work_files = []
        target_files = 100  # Target total files
        files_per_dir = 10  # Take up to 10 files from each directory

        # Calculate how many directories we need to sample
        # We'll sample more than needed to account for directories with fewer files
        num_dirs_to_sample = 20  # Sample from 20 directories to ensure we get ~100 files

        # Calculate step size to evenly distribute across date range
        step = max(1, len(date_dirs) // num_dirs_to_sample)

        sampled_dirs = []
        for i in range(0, len(date_dirs), step)[:num_dirs_to_sample]:
            if len(work_files) >= target_files:
                break  # We have enough files

            date_dir = date_dirs[i]
            files_in_dir = sorted(date_dir.glob('*.gz'))

            if files_in_dir:  # Only add if directory has files
                sampled_dirs.append((date_dir.name, len(files_in_dir)))
                work_files.extend(files_in_dir[:files_per_dir])

        print(f"\n[SAMPLE MODE] IMPROVED DIVERSE SAMPLING:")
        print(f"  Total date directories available: {len(date_dirs)}")
        print(f"  Date range: {date_dirs[0].name} to {date_dirs[-1].name}")
        print(f"  Sampled from {len(sampled_dirs)} directories (every ~{step} directories):")
        for dir_name, file_count in sampled_dirs[:10]:  # Show first 10
            print(f"    - {dir_name} ({file_count} files available)")
        if len(sampled_dirs) > 10:
            print(f"    ... and {len(sampled_dirs) - 10} more directories")
        print(f"  Target files: {target_files}, Actual files sampled: {len(work_files)}")
    else:
...
def integrate_openalex_improved(max_works_per_tech=10000, strictness='moderate', sample_mode=False):
    """
    Main integration function - VERSION 5 with NULL data-driven pattern expansion
    Uses V4 NULL data captures to expand both Stage 1 (keywords) and Stage 2 (topics)
    Expected 2-3x improvement: 30,000-45,000 works vs V4's 14,955 works
    """

    print("=" * 80)
    print("OPENALEX COMPREHENSIVE INTEGRATION - VERSION 5 (NULL DATA-DRIVEN EXPANSION)")
    print("=" * 80)
    print()
    print(f"Validation strictness: {strictness}")
