"""
Comprehensive OpenAlex FULL Integration to Master Database
PRODUCTION MODE: Processes ALL 2,938 files with 10K works per technology
Extracts: works, authors, institutions, funders, countries for 9 technology domains
"""
...
def process_all_works_data(technology_domains, max_works_per_tech=10000):
    """
    PRODUCTION: Process ALL OpenAlex files for specified technologies
    max_works_per_tech: 10,000 works per technology (90K total)
    """

    print(f"\n{'=' * 80}")
    print("FULL OPENALEX PROCESSING - PRODUCTION MODE")
    print(f"{'=' * 80}")
    ...
    print(f"\nFound {len(work_files):,} work files in OpenAlex dataset")
    print(f"Processing ALL {len(work_files):,} files...\n")
    ...
    for file_path in work_files:
                    ...
                    try:
                        ...
                        for tech_name, keywords in technology_domains.items():
                            ...
                            if matches_technology(combined_text, keywords):
                                ...
                                # Only print first 100 matches per tech to avoid spam
                                if len(works_by_tech[tech_name]) <= 100:
                                    print(f"  [{tech_name}] #{len(works_by_tech[tech_name])}: {title[:50]}...")
        ...
        if files_processed % 100 == 0:
            ...
            for tech, works in sorted(works_by_tech.items()):
                ...
                print(f"  {tech:20s}: {len(works):6,} / {max_works_per_tech:6,} ({pct:5.1f}%)")
            print()
    ...
    print()
    for tech, works in sorted(works_by_tech.items()):
        print(f"  {tech}: {len(works):,} works extracted")

    return works_by_tech

def integrate_openalex_full():
    """
    Main integration function - PRODUCTION MODE
    Processes ALL OpenAlex files with 10K works per technology
    """

    print("=" * 80)
    print("OPENALEX COMPREHENSIVE INTEGRATION - PRODUCTION MODE")
    print("=" * 80)
    print()
    print("Processing ALL 2,938 OpenAlex files")
    print("Target: 10,000 works per technology = 90,000 total works")
    print("Est. time: 2-4 hours")
    print()
    ...
    works_by_tech = process_all_works_data(TECHNOLOGY_KEYWORDS, max_works_per_tech=10000)
    ...
    for tech_name, works in works_by_tech.items():
        ...
        for idx, work in enumerate(works):
            if idx % 1000 == 0 and idx > 0:
                print(f"  Inserted {idx:,} / {len(works):,} works...", end='\r')
        ...
        """, (
            ...
            len(funders_seen),
            'OpenAlex Snapshot (Full Processing)',
            'Full integration - all files processed'
        ))
    ...
    print(f"\n{'=' * 80}")
    print("[OK] FULL PRODUCTION INTEGRATION COMPLETE")
    print(f"{'=' * 80}")
