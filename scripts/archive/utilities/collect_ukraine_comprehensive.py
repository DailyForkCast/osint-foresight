#!/usr/bin/env python3
"""
Collect Ukraine-China Events - Comprehensive Geographic Search
Third largest European partner - 190K missing events
"""
...
print("\n" + "=" * 100)
print("UKRAINE-CHINA COMPREHENSIVE COLLECTION")
print("=" * 100)
print("Expected: 290,735 total events (currently have 100,232 - missing 190,503)")
print("=" * 100)
...
query = """
    ...
    WHERE (
        Actor1CountryCode IN ('UKR', 'UA') OR Actor2CountryCode IN ('UKR', 'UA')
        OR LOWER(Actor1Name) LIKE '%ukraine%' OR LOWER(Actor2Name) LIKE '%ukraine%'
        OR LOWER(Actor1Name) LIKE '%ukrainian%' OR LOWER(Actor2Name) LIKE '%ukrainian%'
        OR LOWER(Actor1Name) LIKE '%kyiv%' OR LOWER(Actor2Name) LIKE '%kyiv%'
        OR LOWER(Actor1Name) LIKE '%kiev%' OR LOWER(Actor2Name) LIKE '%kiev%'
        OR Actor1Geo_CountryCode IN ('UKR', 'UA') OR Actor2Geo_CountryCode IN ('UKR', 'UA')
        OR LOWER(Actor1Geo_FullName) LIKE '%ukraine%' OR LOWER(Actor2Geo_FullName) LIKE '%ukraine%'
        OR LOWER(Actor1Geo_FullName) LIKE '%kyiv%' OR LOWER(Actor2Geo_FullName) LIKE '%kyiv%'
        OR ActionGeo_CountryCode IN ('UKR', 'UA')
        OR LOWER(ActionGeo_FullName) LIKE '%ukraine%'
        OR LOWER(ActionGeo_FullName) LIKE '%kyiv%'
    )
...
print(f"Found {len(results):,} Ukraine-China events")
...
for row in results:
    matched_fields = []
    if row.Actor1CountryCode in ('UKR', 'UA') or row.Actor2CountryCode in ('UKR', 'UA'):
        matched_fields.append('actor_country_code')
        field_stats['actor_country_code'] += 1
    if (row.Actor1Name and any(x in row.Actor1Name.lower() for x in ['ukraine', 'ukrainian', 'kyiv', 'kiev'])) or \
       (row.Actor2Name and any(x in row.Actor2Name.lower() for x in ['ukraine', 'ukrainian', 'kyiv', 'kiev'])):
        matched_fields.append('actor_name')
        field_stats['actor_name'] += 1
    if row.Actor1Geo_CountryCode in ('UKR', 'UA') or row.Actor2Geo_CountryCode in ('UKR', 'UA'):
        matched_fields.append('actor_geo_country_code')
        field_stats['actor_geo_country_code'] += 1
    if (row.Actor1Geo_FullName and any(x in row.Actor1Geo_FullName.lower() for x in ['ukraine', 'kyiv'])) or \
       (row.Actor2Geo_FullName and any(x in row.Actor2Geo_FullName.lower() for x in ['ukraine', 'kyiv'])):
        matched_fields.append('actor_geo_full_name')
        field_stats['actor_geo_full_name'] += 1
    if row.ActionGeo_CountryCode in ('UKR', 'UA'):
        matched_fields.append('action_geo_country_code')
        field_stats['action_geo_country_code'] += 1
    if row.ActionGeo_FullName and any(x in row.ActionGeo_FullName.lower() for x in ['ukraine', 'kyiv']):
...
output_file = 'analysis/ukraine_china_comprehensive_events.json'
with open(output_file, 'w') as f:
    json.dump({'collection_timestamp': datetime.now().isoformat(), 'country': 'Ukraine',
...
print(f"\nUKRAINE COLLECTION COMPLETE: {len(events):,} events")
