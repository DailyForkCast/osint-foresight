#!/usr/bin/env python3
"""
Collect Italy-China Events - Comprehensive Geographic Search
Tier 2: 58K missing events
"""
...
print("\n" + "=" * 100)
print("ITALY-CHINA COMPREHENSIVE COLLECTION")
print("=" * 100)
print("Expected: 97,316 total events (currently have 39,110 - missing 58,206)")
print("=" * 100)
...
query = """
    ...
    WHERE (
        Actor1CountryCode IN ('ITA', 'IT') OR Actor2CountryCode IN ('ITA', 'IT')
        OR LOWER(Actor1Name) LIKE '%italy%' OR LOWER(Actor2Name) LIKE '%italy%'
        OR LOWER(Actor1Name) LIKE '%italian%' OR LOWER(Actor2Name) LIKE '%italian%'
        OR LOWER(Actor1Name) LIKE '%rome%' OR LOWER(Actor2Name) LIKE '%rome%'
        OR Actor1Geo_CountryCode IN ('ITA', 'IT') OR Actor2Geo_CountryCode IN ('ITA', 'IT')
        OR LOWER(Actor1Geo_FullName) LIKE '%italy%' OR LOWER(Actor2Geo_FullName) LIKE '%italy%'
        OR LOWER(Actor1Geo_FullName) LIKE '%rome%' OR LOWER(Actor2Geo_FullName) LIKE '%rome%'
        OR ActionGeo_CountryCode IN ('ITA', 'IT')
        OR LOWER(ActionGeo_FullName) LIKE '%italy%'
        OR LOWER(ActionGeo_FullName) LIKE '%rome%'
    )
...
print(f"Found {len(results):,} Italy-China events")
...
for row in results:
    matched_fields = []
    if row.Actor1CountryCode in ('ITA', 'IT') or row.Actor2CountryCode in ('ITA', 'IT'):
        matched_fields.append('actor_country_code')
        field_stats['actor_country_code'] += 1
    if (row.Actor1Name and any(x in row.Actor1Name.lower() for x in ['italy', 'italian', 'rome'])) or \
       (row.Actor2Name and any(x in row.Actor2Name.lower() for x in ['italy', 'italian', 'rome'])):
        matched_fields.append('actor_name')
        field_stats['actor_name'] += 1
    if row.Actor1Geo_CountryCode in ('ITA', 'IT') or row.Actor2Geo_CountryCode in ('ITA', 'IT'):
        matched_fields.append('actor_geo_country_code')
        field_stats['actor_geo_country_code'] += 1
    if (row.Actor1Geo_FullName and any(x in row.Actor1Geo_FullName.lower() for x in ['italy', 'rome'])) or \
       (row.Actor2Geo_FullName and any(x in row.Actor2Geo_FullName.lower() for x in ['italy', 'rome'])):
        matched_fields.append('actor_geo_full_name')
        field_stats['actor_geo_full_name'] += 1
    if row.ActionGeo_CountryCode in ('ITA', 'IT'):
        matched_fields.append('action_geo_country_code')
        field_stats['action_geo_country_code'] += 1
    if row.ActionGeo_FullName and any(x in row.ActionGeo_FullName.lower() for x in ['italy', 'rome']):
...
output_file = 'analysis/italy_china_comprehensive_events.json'
with open(output_file, 'w') as f:
    json.dump({'collection_timestamp': datetime.now().isoformat(), 'country': 'Italy',
...
print(f"\nITALY COLLECTION COMPLETE: {len(events):,} events")
