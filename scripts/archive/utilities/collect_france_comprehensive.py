#!/usr/bin/env python3
"""
Collect France-China Events - Comprehensive Geographic Search
Fourth largest European partner - 102K missing events
"""
...
print("\n" + "=" * 100)
print("FRANCE-CHINA COMPREHENSIVE COLLECTION")
print("=" * 100)
print("Expected: 172,511 total events (currently have 70,166 - missing 102,345)")
print("=" * 100)
...
query = """
    ...
    WHERE (
        Actor1CountryCode IN ('FRA', 'FR') OR Actor2CountryCode IN ('FRA', 'FR')
        OR LOWER(Actor1Name) LIKE '%france%' OR LOWER(Actor2Name) LIKE '%france%'
        OR LOWER(Actor1Name) LIKE '%french%' OR LOWER(Actor2Name) LIKE '%french%'
        OR LOWER(Actor1Name) LIKE '%paris%' OR LOWER(Actor2Name) LIKE '%paris%'
        OR Actor1Geo_CountryCode IN ('FRA', 'FR') OR Actor2Geo_CountryCode IN ('FRA', 'FR')
        OR LOWER(Actor1Geo_FullName) LIKE '%france%' OR LOWER(Actor2Geo_FullName) LIKE '%france%'
        OR LOWER(Actor1Geo_FullName) LIKE '%paris%' OR LOWER(Actor2Geo_FullName) LIKE '%paris%'
        OR ActionGeo_CountryCode IN ('FRA', 'FR')
        OR LOWER(ActionGeo_FullName) LIKE '%france%'
        OR LOWER(ActionGeo_FullName) LIKE '%paris%'
    )
...
print(f"Found {len(results):,} France-China events")
...
for row in results:
    matched_fields = []
    if row.Actor1CountryCode in ('FRA', 'FR') or row.Actor2CountryCode in ('FRA', 'FR'):
        matched_fields.append('actor_country_code')
        field_stats['actor_country_code'] += 1
    if (row.Actor1Name and any(x in row.Actor1Name.lower() for x in ['france', 'french', 'paris'])) or \
       (row.Actor2Name and any(x in row.Actor2Name.lower() for x in ['france', 'french', 'paris'])):
        matched_fields.append('actor_name')
        field_stats['actor_name'] += 1
    if row.Actor1Geo_CountryCode in ('FRA', 'FR') or row.Actor2Geo_CountryCode in ('FRA', 'FR'):
        matched_fields.append('actor_geo_country_code')
        field_stats['actor_geo_country_code'] += 1
    if (row.Actor1Geo_FullName and any(x in row.Actor1Geo_FullName.lower() for x in ['france', 'paris'])) or \
       (row.Actor2Geo_FullName and any(x in row.Actor2Geo_FullName.lower() for x in ['france', 'paris'])):
        matched_fields.append('actor_geo_full_name')
        field_stats['actor_geo_full_name'] += 1
    if row.ActionGeo_CountryCode in ('FRA', 'FR'):
        matched_fields.append('action_geo_country_code')
        field_stats['action_geo_country_code'] += 1
    if row.ActionGeo_FullName and any(x in row.ActionGeo_FullName.lower() for x in ['france', 'paris']):
...
output_file = 'analysis/france_china_comprehensive_events.json'
with open(output_file, 'w') as f:
    json.dump({'collection_timestamp': datetime.now().isoformat(), 'country': 'France',
...
print(f"\nFRANCE COLLECTION COMPLETE: {len(events):,} events")
