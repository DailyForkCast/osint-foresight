#!/usr/bin/env python3
"""
Industry-Specific Validation - Priority 1, Recommendation 4

Validates entities using domain-appropriate databases:
- USPTO patents for technology companies
- OpenAlex research for R&D entities

Expected: 25-40% validation rate (vs. 14.5% from public procurement)
"""
...
print("="*80)
print("INDUSTRY-SPECIFIC VALIDATION")
print("="*80)
print()
...
print()
print("Expected improvement: 14.5% -> 25-40% validation rate")
print()
...
for entity in historical_data['entities']:
    ...
    for term in search_terms[:3]:  # Limit to top 3 terms for speed
        try:
            # USAspending
            cursor.execute("""
    ...
    for term in search_terms:
        try:
            ...
            # Get examples
            if count > 0 and len(finding['patents']['examples']) < 3:
                cursor.execute("""
                    SELECT patent_number, title, grant_date, assignee_name
    ...
    # 3. CHECK OPENALEX RESEARCH
    research_papers = 0
    total_citations = 0

    for term in search_terms:
        try:
            # Check if entity exists in OpenAlex entities
            cursor.execute("""
                SELECT works_count, cited_by_count, h_index
            ...
            row = cursor.fetchone()
            if row:
                research_papers += row['works_count'] or 0
                total_citations += row['cited_by_count'] or 0

            # Get example papers
            if len(finding['research']['examples']) < 3:
                cursor.execute("""
    ...
    details = []
    if finding['procurement']['usaspending'] > 0 or finding['procurement']['ted'] > 0:
        details.append(f"Procurement: {finding['procurement']['usaspending']} USA, {finding['procurement']['ted']} TED")
    if patent_count > 0:
        details.append(f"Patents: {patent_count:,}")
    if research_papers > 0:
        details.append(f"Research: {research_papers:,} papers")
...
print(f"  Total patents: {patents['total_patents']:,}")
...
for sector, stats in sorted(results['by_sector'].items(), key=lambda x: x[1]['found_any'], reverse=True):
    if stats['total'] > 0:
        pct = stats['found_any'] / stats['total'] * 100
        print(f"{sector:30} {stats['found_any']:2}/{stats['total']:2} ({pct:5.1f}%) - Proc:{stats['found_procurement']:2} Pat:{stats['found_patents']:2} Res:{stats['found_research']:2}")

# Save results
output_path = ANALYSIS_DIR / f"industry_specific_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
