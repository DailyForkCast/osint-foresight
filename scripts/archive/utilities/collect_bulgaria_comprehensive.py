#!/usr/bin/env python3
"""
Collect Bulgaria-China Events - Comprehensive Geographic Search
Tier 2: 48K missing events
"""
...
print("\n" + "=" * 100)
print("BULGARIA-CHINA COMPREHENSIVE COLLECTION")
print("=" * 100)
print("Expected: 50,459 total events (currently have 2,155 - missing 48,304)")
print("=" * 100)
...
query = """
    ...
    WHERE (
        ...
        OR LOWER(Actor1Name) LIKE '%sofia%' OR LOWER(Actor2Name) LIKE '%sofia%'
        ...
        OR LOWER(Actor1Geo_FullName) LIKE '%sofia%' OR LOWER(Actor2Geo_FullName) LIKE '%sofia%'
        OR ActionGeo_CountryCode IN ('BGR', 'BG')
        OR LOWER(ActionGeo_FullName) LIKE '%bulgaria%'
        OR LOWER(ActionGeo_FullName) LIKE '%sofia%'
    )
...
print(f"Found {len(results):,} Bulgaria-China events")
...
for row in results:
    matched_fields = []
    if row.Actor1CountryCode in ('BGR', 'BG') or row.Actor2CountryCode in ('BGR', 'BG'):
        matched_fields.append('actor_country_code')
        field_stats['actor_country_code'] += 1
    if (row.Actor1Name and any(x in row.Actor1Name.lower() for x in ['bulgaria', 'bulgarian', 'sofia'])) or \
       (row.Actor2Name and any(x in row.Actor2Name.lower() for x in ['bulgaria', 'bulgarian', 'sofia'])):
        matched_fields.append('actor_name')
        field_stats['actor_name'] += 1
    if row.Actor1Geo_CountryCode in ('BGR', 'BG') or row.Actor2Geo_CountryCode in ('BGR', 'BG'):
        matched_fields.append('actor_geo_country_code')
        field_stats['actor_geo_country_code'] += 1
    if (row.Actor1Geo_FullName and any(x in row.Actor1Geo_FullName.lower() for x in ['bulgaria', 'sofia'])) or \
       (row.Actor2Geo_FullName and any(x in row.Actor2Geo_FullName.lower() for x in ['bulgaria', 'sofia'])):
        matched_fields.append('actor_geo_full_name')
        field_stats['actor_geo_full_name'] += 1
    if row.ActionGeo_CountryCode in ('BGR', 'BG'):
        matched_fields.append('action_geo_country_code')
        field_stats['action_geo_country_code'] += 1
    if row.ActionGeo_FullName and any(x in row.ActionGeo_FullName.lower() for x in ['bulgaria', 'sofia']):
...
output_file = 'analysis/bulgaria_china_comprehensive_events.json'
with open(output_file, 'w') as f:
    json.dump({'collection_timestamp': datetime.now().isoformat(), 'country': 'Bulgaria',
...
print(f"\nBULGARIA COLLECTION COMPLETE: {len(events):,} events")
