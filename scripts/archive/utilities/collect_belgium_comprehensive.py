#!/usr/bin/env python3
"""
Collect Belgium-China Events - Comprehensive Geographic Search
Tier 2: 20K missing events
"""
...
print("\n" + "=" * 100)
print("BELGIUM-CHINA COMPREHENSIVE COLLECTION")
print("=" * 100)
print("Expected: 34,555 total events (currently have 14,501 - missing 20,054)")
print("=" * 100)
...
query = """
    ...
    WHERE (
        Actor1CountryCode IN ('BEL', 'BE') OR Actor2CountryCode IN ('BEL', 'BE')
        OR LOWER(Actor1Name) LIKE '%belgium%' OR LOWER(Actor2Name) LIKE '%belgium%'
        OR LOWER(Actor1Name) LIKE '%belgian%' OR LOWER(Actor2Name) LIKE '%belgian%'
        OR LOWER(Actor1Name) LIKE '%brussels%' OR LOWER(Actor2Name) LIKE '%brussels%'
        OR Actor1Geo_CountryCode IN ('BEL', 'BE') OR Actor2Geo_CountryCode IN ('BEL', 'BE')
        OR LOWER(Actor1Geo_FullName) LIKE '%belgium%' OR LOWER(Actor2Geo_FullName) LIKE '%belgium%'
        OR LOWER(Actor1Geo_FullName) LIKE '%brussels%' OR LOWER(Actor2Geo_FullName) LIKE '%brussels%'
        OR ActionGeo_CountryCode IN ('BEL', 'BE')
        OR LOWER(ActionGeo_FullName) LIKE '%belgium%'
        OR LOWER(ActionGeo_FullName) LIKE '%brussels%'
    )
...
print(f"Found {len(results):,} Belgium-China events")
...
for row in results:
    matched_fields = []
    if row.Actor1CountryCode in ('BEL', 'BE') or row.Actor2CountryCode in ('BEL', 'BE'):
        matched_fields.append('actor_country_code')
        field_stats['actor_country_code'] += 1
    if (row.Actor1Name and any(x in row.Actor1Name.lower() for x in ['belgium', 'belgian', 'brussels'])) or \
       (row.Actor2Name and any(x in row.Actor2Name.lower() for x in ['belgium', 'belgian', 'brussels'])):
        matched_fields.append('actor_name')
        field_stats['actor_name'] += 1
    if row.Actor1Geo_CountryCode in ('BEL', 'BE') or row.Actor2Geo_CountryCode in ('BEL', 'BE'):
        matched_fields.append('actor_geo_country_code')
        field_stats['actor_geo_country_code'] += 1
    if (row.Actor1Geo_FullName and any(x in row.Actor1Geo_FullName.lower() for x in ['belgium', 'brussels'])) or \
       (row.Actor2Geo_FullName and any(x in row.Actor2Geo_FullName.lower() for x in ['belgium', 'brussels'])):
        matched_fields.append('actor_geo_full_name')
        field_stats['actor_geo_full_name'] += 1
    if row.ActionGeo_CountryCode in ('BEL', 'BE'):
        matched_fields.append('action_geo_country_code')
        field_stats['action_geo_country_code'] += 1
    if row.ActionGeo_FullName and any(x in row.ActionGeo_FullName.lower() for x in ['belgium', 'brussels']):
...
output_file = 'analysis/belgium_china_comprehensive_events.json'
with open(output_file, 'w') as f:
    json.dump({'collection_timestamp': datetime.now().isoformat(), 'country': 'Belgium',
...
print(f"\nBELGIUM COLLECTION COMPLETE: {len(events):,} events")
