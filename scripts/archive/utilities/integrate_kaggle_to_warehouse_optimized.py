#!/usr/bin/env python3
"""
OPTIMIZED: Integrate Kaggle arXiv expanded database into master warehouse.
Handles: papers, authors, categories, technology classifications, statistics.

Key optimization: Pre-compute top technology using indexed JOIN instead of correlated subquery.
"""
...
print('=' * 80)
print('KAGGLE ARXIV -> MASTER WAREHOUSE INTEGRATION (OPTIMIZED)')
print('=' * 80)
...
# ============================================================================
# STEP 3: Pre-compute top technology per paper (OPTIMIZATION)
# ============================================================================
print('\n[3/7] Pre-computing top technology per paper...')
print('  Creating optimized lookup table (this will take ~30 seconds)...')

start_time = time.time()

# Create index if not exists (speeds up GROUP BY dramatically)
try:
    cursor_kaggle.execute('CREATE INDEX IF NOT EXISTS idx_tech_score ON kaggle_arxiv_technology(arxiv_id, match_score DESC)')
    conn_kaggle.commit()
    print('  [OK] Index created/verified')
except Exception as e:
    print(f'  [WARN] Index creation skipped: {e}')

# Create temporary table with top technology per paper
cursor_kaggle.execute('DROP TABLE IF EXISTS temp_top_tech')
...
cursor_kaggle.execute('SELECT COUNT(*) FROM temp_top_tech')
top_tech_count = cursor_kaggle.fetchone()[0]

print(f'  [OK] Pre-computed {top_tech_count:,} top technologies in {precompute_elapsed:.1f} seconds')

# ============================================================================
# STEP 4: Integrate Papers (OPTIMIZED)
# ============================================================================
print('\n[4/7] Integrating papers...')
print('  This will take ~2-3 minutes for 1.4M papers...')
...
cursor_kaggle.execute('''
    SELECT
        ...
        p.abstract,
        p.primary_category,
        COALESCE(t.technology_domain, 'Unknown') as top_technology
...
# ============================================================================
# STEP 5: Integrate Authors
# ============================================================================
print('\n[5/7] Integrating authors...')
print('  This will take ~3-5 minutes for 7.6M author records...')
...
# ============================================================================
# STEP 6: Integrate Categories
# ============================================================================
print('\n[6/7] Integrating categories...')
start_time = time.time()
...
# ============================================================================
# STEP 7: Update Statistics and Metadata
# ============================================================================
print('\n[7/7] Updating statistics and metadata...')
...
total_elapsed = precompute_elapsed + papers_elapsed + authors_elapsed + categories_elapsed
...
print('\n' + '=' * 80)
print('[OK] INTEGRATION SUCCESSFUL - OPTIMIZED VERSION')
print('=' * 80)
