...
class GLEIFREPEXProcessor:
    ...
    def extract_array_values(self, arr, default=''):        """Extract values from array of nested objects"""        if not arr:            return default        if isinstance(arr, list):            values = []            for item in arr:                val = self.extract_nested_value(item)                if val:                    values.append(val)            return ', '.join(values) if values else default        return self.extract_nested_value(arr, default)
