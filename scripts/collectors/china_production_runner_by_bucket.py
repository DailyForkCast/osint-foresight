#!/usr/bin/env python3
"""
China Policy Collector - Production Runner (By Bucket)
Complete pipeline: discovery → download → extraction → QA
Usage: python china_production_runner_by_bucket.py <bucket_name>
"""
        ...
        for source in sources:
            ...
            try:
                ...
                for doc in discovered:
                    ...
                    # Rate limiting
                    time.sleep(2)  # Be polite to Wayback Machine
...
def main():
    """Main entry point"""
    if len(sys.argv) < 2:
        print("Usage: python china_production_runner_by_bucket.py <bucket_name> [max_sources] [max_docs_per_source]")
        print("\nAvailable buckets:")
        print("  SECONDARY - US/European aggregators (safe, direct access)")
        print("  CENTRAL_GOV - Chinese central government (Wayback Machine)")
        print("  MINISTRIES - Chinese ministries (Wayback Machine)")
        print("  DIPLOMATIC_EMBASSIES - Chinese embassies worldwide (Wayback Machine)")
        print("  DIPLOMATIC_TECH_HUB_CONSULATES - Chinese consulates in tech hubs (Wayback Machine)")
        sys.exit(1)

    bucket_name = sys.argv[1]
    max_sources = int(sys.argv[2]) if len(sys.argv) > 2 else None
    max_docs_per_source = int(sys.argv[3]) if len(sys.argv) > 3 else None

    runner = FullExtractionRunner(bucket_name=bucket_name)

    if max_sources or max_docs_per_source:
        logger.info(f"TEST MODE: max_sources={max_sources}, max_docs_per_source={max_docs_per_source}")
    else:
        logger.info("PRODUCTION MODE: Processing ALL sources, ALL documents")

    logger.info(f"Target bucket: {bucket_name}")
    logger.info("=" * 80)

    stats = runner.run_collection(max_sources=max_sources, max_docs_per_source=max_docs_per_source)
