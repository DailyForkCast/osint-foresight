...
# -*- coding: utf-8 -*-
"""
ETL: Expand bilateral_corporate_links from GLEIF Ownership Trees - V3 (Encoding Fixed)
Purpose: Extract Chinese → European corporate ownership relationships

V3 Changes:
  - Fixed Windows encoding issues
  - Skip printing entity names (contain Chinese characters)
  - Focus on execution, detailed inspection in JSON report
...
class GLEIFCorporateLinksETL:
    ...
    def extract_chinese_european_relationships(self) -> List[Dict]:
        ...
        # Show sample WITHOUT entity names (encoding issues)
        if relationships:
    ...
    def post_etl_validation(self):
        ...
        print(f"\n[OK] Country distribution ({len(countries)} countries):")
    ...
    def save_report(self, total_links: int):
        ...
        report = {
            ...
            "etl_script": "etl_corporate_links_from_gleif_v3_fixed.py",
            ...
            "sample_links": [
                {
                    "link_id": l['link_id'],
                    "parent_lei": l['chinese_entity_lei'],
                    "child_lei": l['foreign_entity_lei'],
                    "child_country": l['foreign_entity_country'],
    ...
    def run(self):
        ...
        print("\n" + "="*80)
        print("GLEIF CORPORATE LINKS ETL - V3 (Encoding Fixed)")
        print("="*80)
