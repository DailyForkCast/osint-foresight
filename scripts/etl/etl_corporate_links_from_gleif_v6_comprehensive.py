...
# -*- coding: utf-8 -*-
"""
ETL: Expand bilateral_corporate_links from GLEIF - V6 COMPREHENSIVE
Purpose: Extract Chinese → European corporate ownership relationships

V6 Changes:
  - Remove LIMIT to find ALL relationships (not just first 10K)
  - Use comprehensive JOIN query (proven in v3/v4)
  - Accept that query will take time, but will find all 170+ relationships
  - Run with patience - indexes will still help vs no indexes
...
class GLEIFCorporateLinksETL:
    ...
    def extract_chinese_european_relationships(self) -> List[Dict]:
        """Extract Chinese → European ownership relationships"""
        print("="*80)
        print("EXTRACTION PHASE - COMPREHENSIVE (NO LIMIT)")
        print("="*80)
        print("NOTE: This query may take 10-30 minutes with 3M entities + 464K relationships")
        print("      Indexes will help but query is complex. Please be patient...")
        print()
        ...
        # COMPREHENSIVE query - NO LIMIT to find ALL relationships
        query = f"""
        SELECT DISTINCT
        ...
        print("\nExecuting extraction query (started at {})...".format(datetime.now().strftime("%H:%M:%S")))
        print("(Query will complete when all relationships are extracted)")
    ...
    def save_report(self, total_links: int):
        ...
        report = {
            ...
            "etl_script": "etl_corporate_links_from_gleif_v6_comprehensive.py",
    ...
    def run(self):
        ...
        print("\n" + "="*80)
        print("GLEIF CORPORATE LINKS ETL - V6 COMPREHENSIVE")
        print("="*80)
