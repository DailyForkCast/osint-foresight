    ...
    def run_collection(self):
        ...
        for category, codes in self.strategic_codes.items():
            ...
            category_totals[category] = {
                'imports': category_imports,
                'exports': category_exports,
                'codes_found': successful
            }

            if successful > 0:
                print(f"\n{category.upper()} TOTALS:")
                print(f"  Total imports from China: EUR {category_imports/1e9:.2f}B")
                print(f"  Total exports to China: EUR {category_exports/1e9:.2f}B")
                print(f"  Products found: {successful}/{len(codes)}")

        # Final summary
        print("\n" + "="*80)
        print("STRATEGIC TRADE SUMMARY")
        print("="*80)

        total_imports = sum(cat['imports'] for cat in category_totals.values())
        total_exports = sum(cat['exports'] for cat in category_totals.values())

        print(f"\nTOTAL STRATEGIC GOODS TRADE:")
        ...
        print(f"  Dependency ratio: {total_imports/total_exports:.1f}:1")

        # Top dependencies
        print(f"\nTOP DEPENDENCIES (by import value):")
        sorted_results = sorted(results.items(), key=lambda x: x[1]['imports'], reverse=True)
        for code, data in sorted_results[:10]:
            hs_desc = None
            for cat_codes in self.strategic_codes.values():
                if code in cat_codes:
                    hs_desc = cat_codes[code]
                    break
            if hs_desc:
                print(f"  {code} - {hs_desc[:40]}")
                print(f"    Imports: EUR {data['imports']/1e9:.2f}B, Ratio: {data['imports']/max(data['exports'],1):.1f}:1")

        # Save summary
        summary_file = self.base_path / f"strategic_trade_summary_{datetime.now().strftime('%Y%m%d')}.txt"
        with open(summary_file, 'w') as f:
            f.write(f"EU-CHINA STRATEGIC TRADE ANALYSIS\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
            f.write("="*60 + "\n\n")

            for category, totals in category_totals.items():
                f.write(f"{category.upper()}:\n")
                f.write(f"  Imports: EUR {totals['imports']/1e9:.2f}B\n")
                f.write(f"  Exports: EUR {totals['exports']/1e9:.2f}B\n")
                f.write(f"  Products: {totals['codes_found']}\n\n")

            f.write(f"\nTOTAL: Imports EUR {total_imports/1e9:.2f}B, Exports EUR {total_exports/1e9:.2f}B\n")
            f.write(f"Trade Deficit: EUR {(total_imports-total_exports)/1e9:.2f}B\n")
            f.write(f"Dependency Ratio: {total_imports/total_exports:.1f}:1\n")

        print(f"\nSummary saved to: {summary_file}")
        print(f"All data saved to: {self.base_path}")
