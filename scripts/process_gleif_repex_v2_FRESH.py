#!/usr/bin/env python3
"""
GLEIF REPEX (Reporting Exceptions) Processor
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gleif_repex_processing.log'),
...
class GLEIFREPEXProcessor:
    ...
    def process_repex_streaming(self):
        ...
        with zipfile.ZipFile(zip_file) as zf:
            ...
            with zf.open(json_filename, 'r') as f:
                ...
                try:
                    # Try 'exceptions' first (most likely)
                    exceptions = ijson.items(f, 'exceptions.item')

                    for exception in exceptions:
                        try:
                            # Extract exception data
                            exception_record = exception.get('ReportingException', exception)

                            lei = self.extract_nested_value(
                                exception_record.get('LEI', {})
                            )

                            exception_info = exception_record.get('ExceptionCategory', {})
                            exception_category = self.extract_nested_value(exception_info)
                            # FIX: Use extract_array_values since ExceptionReason is an array
                            exception_reason = self.extract_array_values(
                                exception_record.get('ExceptionReason', [])
                            )
                            exception_reference = self.extract_nested_value(
                                exception_record.get('ExceptionReference', {})
                            )

                            registration_status = self.extract_nested_value(
                                exception_record.get('Registration', {}).get('RegistrationStatus', {})
                            )

                            last_update = self.extract_nested_value(
                                exception_record.get('Registration', {}).get('LastUpdateDate', {})
                            )
                            ...
                            # Sanitize the record to ensure all values are SQL-compatible types
                            repex_record_clean = tuple(
                                str(v) if not isinstance(v, (str, int, float, type(None))) else v
                                for v in repex_record
                            )

                            batch.append(repex_record_clean)
                            ...
                            logger.warning(f"Error processing exception: {e}")
                            errors += 1
                            continue
    ...
    def run(self):
        ...
        try:
            logger.info("="*80)
            logger.info("GLEIF REPEX PROCESSING")
            logger.info("="*80)
            start_time = time.time()
